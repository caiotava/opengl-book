#ifndef OPENGL_BOOK_SCENEINTERPOLATECUBECOLOR_H
#define OPENGL_BOOK_SCENEINTERPOLATECUBECOLOR_H
...
class SceneIntepolateCubeColor final : public core::Scene {
public:
    SceneIntepolateCubeColor() : Scene("4.2 Interpolate Cube Color") {
    ...
    void OnUpdate(float ts) override {
        ...
        const auto translatedMatrix = glm::translate(
            ...
            glm::vec3(
                // 0.0f,
                std::sin(0.35f * ts) * 2.0f,
                // 0.0f,
                std::sin(0.52f * ts) * 2.0f,
                // 0.0f
                std::sin(0.75f * ts) * 2.0f
            )
        );

        auto rotationMatrix = glm::mat4(1.0f);
        rotationMatrix = glm::rotate(rotationMatrix, 1.75f * ts, glm::vec3(1.0f, 0.0f, 0.0f));
        rotationMatrix = glm::rotate(rotationMatrix, 1.75f * ts, glm::vec3(0.0f, 1.0f, 0.0f));
        rotationMatrix = glm::rotate(rotationMatrix, 1.75f * ts, glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    void OnRender() override {
        ...
        auto bufferSize = core::Application::Get().GetFramebufferSize();
        ...
        // m_modelMatrix = glm::translate(glm::mat4(1.0f), m_cubePosition);
...
#endif //OPENGL_BOOK_SCENEINTERPOLATECUBECOLOR_H
